...
export default function Home() {
  ...
  const fetchExistingData = async () => {
    try {
      const res = await fetch(`${API_URL}/projects`);   //http://localhost:5000/api
  ...
  const handleScan = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/projects/scan`, {
        method: 'POST',
  ...
  const handleClear = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/projects/clear`, { method: 'DELETE' });
      if (res.ok) {
        setProjects([]);
      }
    } catch (error) {
